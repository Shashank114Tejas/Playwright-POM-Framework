...
for (const data of dataset) {
  test(`Valid login, Add product to cart and Place order of ${data.productName}`, async ({ page }) => {
    //login page
    ...
    //MycartPage
    const flag = await page.locator("h3:has-text('" + data.productName + "')").isVisible();
    ...
    await checkoutpage.selectShippingCountryAndPlaceOrder("ind", " India", data.username)
...
customtest.only("Passing test data as fixture from test base" , async ({ page,testDataForOrder }) => {
  //login page
  const poManager = new POManager(page);
  const loginPage = poManager.getLoginPage();
  await loginPage.goto();
